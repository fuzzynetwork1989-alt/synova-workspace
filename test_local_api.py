#!/usr/bin/env python3
"""
Local test for Enhanced Synova Brain API
Tests the simplified API locally before Railway deployment
"""
...
class LocalTester:
    ...
    async def test_health_check(self):
        ...
        try:
            ...
            print("   Make sure the API is running: python main_simple.py")
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(f"{self.base_url}/generate/stream", json=payload) as response:
                    if response.status == 200:
                        ...
                        chunks = content.count("data:")
                        print("✅ Streaming Generation PASSED")
                        print(f"   Chunks received: {chunks}")
                        self.results.append({"test": "streaming", "status": "PASS", "chunks": chunks})
                        return True
                    else:
    ...
    async def run_all_tests(self):
        """Run all local tests"""
        print("🚀 Starting Local Tests for Enhanced Synova Brain")
        ...
        if passed == total:
            print("🎉 ALL TESTS PASSED - API Ready for Railway Deployment!")
        else:
        ...
        print(f"📄 Results saved to local_test_results.json")
        return results_data
